...
char* chuanHoaXau(char* xau) {
    ...
    // Loáº¡i bá» khoáº£ng tráº¯ng thá»«a á»Ÿ Ä‘áº§u xĂ¢u
    while (xau[j] == ' ') {
        j++;
    }
    if (j > 0) {
    ...
    // Loáº¡i bá» khoáº£ng tráº¯ng thá»«a á»Ÿ cuá»‘i xĂ¢u
    i = length - 1;
    while (xau[i] == ' ') {
        xau[i] = '\0';
        i--;
    ...
    // Chuáº©n hĂ³a khoáº£ng tráº¯ng giá»¯a cĂ¡c tá»«
    for (i = 0; i < length; i++) {
        if (xau[i] == ' ' && xau[i + 1] == ' ') {
            int j = i;
            while (xau[j] != '\0') {
    ...
    // Viáº¿t hoa chá»¯ cĂ¡i Ä‘áº§u má»—i tá»«
    for (i = 0; i < length; i++) {
        if (i == 0 || xau[i - 1] == ' ') {
            if (xau[i] >= 'a' && xau[i] <= 'z') {
                xau[i] = xau[i] - 32;
